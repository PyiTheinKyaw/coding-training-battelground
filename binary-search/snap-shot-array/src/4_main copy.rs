...
impl Snapshot {
    ...
    fn retrieve(&self,snap_id: usize, current_node: usize) -> i32 {
        let current_node = self.cache[current_node];

        if current_node.0 == snap_id {
            return current_node.1;
        } 
        else if current_node.0 < snap_id { 
            return -1;
        }
        else if current_node.0 > snap_id {
            let left = if self.has_left(current_node.0) { 
                Self::get_left_child(current_node.0) 
            } else {0};

            let right = if self.has_right(current_node.0) {
                Self::get_right_child(current_node.0) 
            } else {0};

            if self.cache[left].0 >= snap_id {
                self.retrieve(snap_id, left);
            }
...
impl SnapshotArray {
    ...
    fn get(&self, index: i32, snap_id: i32) -> i32 {
        ...
        if let Some(snapshot) = self.state.get(index) {
            

            }
            
            // if snapshot.has_left(parent.0) {
            //     let left = Snapshot::get_left_child(parent.0);
            //     if left == snap_id { 
            //         return snapshot.cache[left].1
            //     }
            // }
